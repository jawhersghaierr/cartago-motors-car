      ...
      {settings.google_review_url && (
        <section className="py-12 bg-carbon-100/60 dark:bg-carbon-900/40">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {/* Cartes d'avis manuels */}
            {(() => {
              const reviews = [
                { author: settings.google_review_1_author, text: settings.google_review_1_text, rating: parseInt(settings.google_review_1_rating ?? '5') },
                { author: settings.google_review_2_author, text: settings.google_review_2_text, rating: parseInt(settings.google_review_2_rating ?? '5') },
                { author: settings.google_review_3_author, text: settings.google_review_3_text, rating: parseInt(settings.google_review_3_rating ?? '5') },
              ].filter(r => r.text)
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  {reviews.map((r, i) => (
                    <div key={i} className="bg-white dark:bg-carbon-900 rounded-2xl border border-carbon-200 dark:border-white/10 p-6 flex flex-col gap-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gold-100 dark:bg-gold-900/30 flex items-center justify-center font-bold text-gold-700 dark:text-gold-400 text-sm flex-shrink-0">
                          {(r.author ?? 'G')[0].toUpperCase()}
                        </div>
                        <div>
                          <p className="font-semibold text-carbon-950 dark:text-white text-sm">{r.author ?? 'Client'}</p>
                          <p className="text-carbon-400 text-xs">Avis Google vérifié</p>
                        </div>
                      </div>
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map(n => (
                          <svg key={n} width="14" height="14" viewBox="0 0 24 24" fill={n <= r.rating ? '#FBBC05' : '#e2e8f0'}>
                            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                          </svg>
                        ))}
                      </div>
