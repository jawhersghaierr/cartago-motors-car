      ...
      {settings.google_review_url && (
        <section className="pt-10 pb-4 bg-carbon-100/60 dark:bg-carbon-900/40">
          <div className="max-w-2xl mx-auto px-4 text-center">
            <p className="text-gold-700 dark:text-gold-500 text-sm uppercase tracking-widest font-medium mb-3">{t('reviewsLabel')}</p>
            <h2 className="text-3xl font-bold text-carbon-950 dark:text-white mb-8">{t('reviewsTitle')}</h2>
            <div className="inline-flex flex-col items-center gap-4 bg-white dark:bg-carbon-900 rounded-2xl border border-carbon-200 dark:border-white/10 px-10 py-8 shadow-sm">
              <div className="flex items-center gap-3">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
                <div className="flex gap-0.5">
                  {[1,2,3,4,5].map(i => <svg key={i} width="22" height="22" viewBox="0 0 24 24" fill="#FBBC05"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>)}
                </div>
              </div>
              ...
              {settings.google_review_count && (
                <p className="text-carbon-500 dark:text-carbon-400 text-sm">
                  {t('basedOn')} <span className="font-semibold text-carbon-700 dark:text-carbon-300">{settings.google_review_count} {t('googleReviews')}</span>
                </p>
              )}
              <a href={settings.google_review_url} target="_blank" rel="noopener noreferrer" className="btn-gold inline-flex items-center gap-2 px-6 py-3 text-sm mt-2">
                {t('leaveReview')}
