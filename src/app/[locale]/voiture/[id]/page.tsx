...
export default async function VoiturePage({ params }: { params: { locale: string; id: string } }) {
  ...
  const statusClass = c.status === 'available'
          ...
          <div className="grid lg:grid-cols-2 gap-10">
            ...
            <div>
                ...
                <div className="flex flex-wrap gap-4 mb-6 pb-5 border-b border-carbon-100 dark:border-white/5">
                  {(c.price_ht || c.price_ttc) ? (
                    <>
                      {c.price_ht && (
                        <div>
                          <p className="text-xs text-carbon-400 dark:text-carbon-500 uppercase tracking-wider mb-1">{t('priceHT')}</p>
                          <p className="text-2xl font-bold text-gold-gradient">{formatPrice(c.price_ht)}</p>
                        </div>
                      )}
                      {c.price_ttc && (
                        <div>
                          <p className="text-xs text-carbon-400 dark:text-carbon-500 uppercase tracking-wider mb-1">{t('priceTTC')}</p>
                    ...
                    <div>
                      <p className="text-xs text-carbon-400 dark:text-carbon-500 uppercase tracking-wider mb-1">{t('price')}</p>
              ...
              <div className="flex flex-col gap-3 mt-6">
                ...
                {settings.whatsapp && (
                  <a
                    href={`https://wa.me/${settings.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(`Bonjour, je suis intéressé par le véhicule : ${c.brand} ${c.model} (${c.year})`)}`}
                    target="_blank"
              ...
              <p className="text-carbon-400 dark:text-carbon-600 text-xs text-center mt-3">
                {t('addedOn')} {formatDate(c.created_at)}
              </p>
            </div>
          </div>
        </div>
