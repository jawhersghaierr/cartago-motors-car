...
export default async function ContactPage() {
  const [settings, t] = await Promise.all([getSettings(), getTranslations('contact')])

  const contactItems = [
    settings.phone    && { icon: Phone,        label: t('phone'),   value: settings.phone,    href: `tel:${settings.phone}` },
      ...
      <div className="pt-24 pb-20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="mb-10 text-center">
            <p className="text-gold-700 dark:text-gold-500 text-sm uppercase tracking-widest font-medium mb-2">{t('label')}</p>
            <h1 className="text-4xl font-bold text-carbon-950 dark:text-white">{t('title')}</h1>
            <p className="text-carbon-500 dark:text-carbon-400 mt-3">{t('subtitle')}</p>
          </div>
