...
import { CAR_BRANDS, FUEL_TYPES, TRANSMISSION_TYPES, type Car } from '@/types/car'
...
export default function CarForm({ car }: CarFormProps) {
  ...
  async function decodeVin() {
    ...
    try {
      ...
      if (data.brand) { setValue('brand', data.brand, { shouldValidate: true }); filled++ }
      if (data.model) { setValue('model', data.model, { shouldValidate: true }); filled++ }
      if (data.year) { setValue('year', data.year, { shouldValidate: true }); filled++ }
      if (data.fuel) { setValue('fuel', data.fuel, { shouldValidate: true }); filled++ }
      if (data.engine) { setValue('engine', data.engine, { shouldValidate: true }); filled++ }
      if (data.transmission) { setValue('transmission', data.transmission, { shouldValidate: true }); filled++ }

      if (filled > 0) {
        toast.success(`${filled} champ${filled > 1 ? 's' : ''} pré-rempli${filled > 1 ? 's' : ''} via VIN`)
      } else {
  ...
  async function onSubmit(data: CreateCarSchema) {
    try {
      ...
      toast.success(isEditing ? 'Voiture modifiée avec succès' : 'Voiture ajoutée avec succès')
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-10">
      ...
      <section className="bg-white rounded-xl border shadow-sm">
        ...
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="model" {...register('model')} placeholder="ex : Série 3 320d" />
      ...
      <section className="bg-white rounded-xl border shadow-sm">
        <div className="px-6 py-4 border-b">
          <h2 className="font-semibold text-slate-800">Numéros d'identification</h2>
        </div>
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={decodingVin || !vinValue || vinValue.trim().length !== 17}
                onClick={decodeVin}
              >
                {decodingVin
            ...
            <p className="text-xs text-muted-foreground">
              Décodage automatique via API NHTSA (gratuit)
            </p>
          </div>
      ...
      <section className="bg-white rounded-xl border shadow-sm">
        <div className="px-6 py-4 border-b">
          <h2 className="font-semibold text-slate-800">Caractéristiques techniques</h2>
        </div>
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <select id="transmission" {...register('transmission')} className={SELECT_CLASS}>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="engine" {...register('engine')} placeholder="ex : 2.0 TDI 150 ch" />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            />
            {errors.horsepower && (
              <p className="text-xs text-destructive">{errors.horsepower.message}</p>
            )}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="color">Couleur</Label>
            <Input id="color" {...register('color')} placeholder="ex : Noir métallisé" />
          </div>
