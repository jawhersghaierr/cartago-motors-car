...
export default function VoitureCard({ car }: VoitureCardProps) {
  ...
  return (
    <div className="group relative bg-white dark:bg-carbon-900 rounded-2xl border border-carbon-200 dark:border-white/5 overflow-hidden hover:border-gold-600/50 dark:hover:border-gold-500/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:hover:shadow-none">

      {/* Favorite + Share buttons — outside Link to avoid invalid HTML nesting */}
      ...
      {/* Compare toggle button — outside Link */}
      <button
        type="button"
        onClick={() => toggle(car.id)}
        aria-label={selected ? 'Retirer de la comparaison' : 'Ajouter à la comparaison'}
        className={`absolute top-2 left-2 z-20 flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-lg border transition-all duration-200 shadow-md ${
          selected
      ...
      {/* Clickable area — the Link wraps only the navigable content */}
